...
function testSendVideo(res) {
  return res.send(
    JSON.stringify({
      Message: "Mensaje con video",
      Type: 1,
      TypeUrl:
        "https://video-links.b-cdn.net/media/videolinks/video/PrismFX.mp4",
      Action: 0,
      Tags: [],
      Note: "",
    })
...
function testActionContinue(res) {
  return res.send(
    JSON.stringify({
...
function testActionWait(res) {
  return res.send(
    JSON.stringify({
      Message: "Espero la respuesta y vuelvo a llamar al endpoint",
      Type: 4,
      TypeUrl: null,
      Action: 1,
      Tags: [],
...
function testActionAssignOperator(res) {
  return res.send(
    JSON.stringify({
...
function testActionEndConversation(res) {
  return res.send(
    JSON.stringify({
